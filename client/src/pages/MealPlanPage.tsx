    ...
    <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col gap-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setWeekStart(currentWeekStart())} className="px-3 py-2 text-sm text-terracotta-700">
            Today
          </button>
        </div>
      </div>
              ...
              {days.map((day) => (
                <div key={`${mealType}-${day}`} className="border rounded-md p-1.5 bg-white min-h-[4rem] flex flex-col gap-1">
                  ...
                  <button
                    ...
                    className="text-xs text-gray-400 hover:text-terracotta-600 text-left"
                  >
                    + Add
                  </button>
                </div>
